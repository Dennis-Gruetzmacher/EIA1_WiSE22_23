...
(function (L02_2_Vogelhaus) {
    ...
    let heightFarTreeArea;
    let heightMiddleTreeArea;
    let heightNearTreeArea;
    let snowmanAreaX;
    let snowmanAreaY;
    let cloudHeight;
    let ColorSpaces = [
    ...
    function drawSun(_sunX, _sunY) {
        let radius1 = 30;
        let radius2 = 130;
        let sunGradient = crc2.createRadialGradient(0, 0, radius1, 0, 0, radius2);
    ...
    function drawMountains() {
        ...
        for (let i = 0; i < randomBetween(5, 10); i++) {
            ...
            for (let j = 0; j < randomBetween(1, 20); j++) {
    ...
    function drawClouds() {
        ...
        for (let i = 0; i < randomBetween(20, 50); i++) {
            ...
            crc2.scale(3, 1);
            let cloudGradient = crc2.createRadialGradient(0, 0, cloudRadius1, 0, 0, cloudRadius2);
    ...
    function drawGround() {
        for (let j = horizonPoint; j <= crc2.canvas.height; j++) {
    ...
    function drawTrees() {
        for (let j = horizonPoint; j <= horizonPoint + heightFarTreeArea; j = j + randomBetween(3, 8)) {
            for (let i = 0; i <= crc2.canvas.width; i = i + randomBetween(5, 10)) {
                crc2.translate(i, j + randomBetween(-3, 3));
                let tree = new Tree(4, 10, 10, 30, 1, 10);
                tree.draw();
                crc2.resetTransform();
            }
        }
        for (let j = horizonPoint + heightFarTreeArea; j <= horizonPoint + heightFarTreeArea + heightMiddleTreeArea; j = j + randomBetween(5, 15)) {
            for (let i = 0; i <= crc2.canvas.width; i = i + randomBetween(5, 20)) {
                crc2.translate(i, j + randomBetween(-3, 3));
                let tree = new Tree(4, 10, 10, 30, 1.5, 20);
                tree.draw();
                crc2.resetTransform();
            }
        }
        for (let j = horizonPoint + heightFarTreeArea + heightMiddleTreeArea; j <= horizonPoint + heightFarTreeArea + heightMiddleTreeArea + heightNearTreeArea; j = j + randomBetween(10, 20)) {
            for (let i = 0; i <= crc2.canvas.width; i = i + randomBetween(10, 25)) {
                crc2.translate(i, j + randomBetween(-3, 3));
                let tree = new Tree(4, 10, 10, 30, 2, 30);
                tree.draw();
                crc2.resetTransform();
            }
        }
    ...
    function drawBirds() {
        ...
        for (let i = 0; i < randomBetween(10, 20); i++) {
            ...
            let bird = new Bird(birdColors[randomBetween(0, 2)][randomBetween(0, 2)], birdColors[randomBetween(0, 2)][randomBetween(0, 2)]);
            bird.draw();
            crc2.resetTransform();
        }
    }
    function drawSnowfall() {
        for (let i = 1; i < randomBetween(400, 750); i++) {
    ...
    function strokeSnowflake(x, y, r, n, inset) {
        crc2.beginPath();
        crc2.translate(x, y);
        let snowGradient = crc2.createRadialGradient(0, 0, 1, 0, 0, r + 2);
...
})(L02_2_Vogelhaus || (L02_2_Vogelhaus = {}));
