...
let globalEndGameState: boolean = false; //Spiel gewonnen (true) verloren (false)
/*Audioelemente*/
...
let globalActiveSentences: Sentence[] = []; // Array für alle im aktuellen Spiel aktiven Sätze (wird beim füllen zufällig gemischt)
...
let fakeWord6: FakeWord =
{ //Flugzeug
    used: false,
    spanish: "avión", 
    ukrainian: "litak"
};
let fakeWord7: FakeWord =
{ //Vogel
    used: false,
    spanish: "pájaro", 
    ukrainian: "ptakh"
};
let fakeWord8: FakeWord =
{ //Raumschiff
    used: false,
    spanish: "astronave", 
    ukrainian: "kosmichnyy korabelʹ"
};
let globalAllFakeWords: FakeWord[] = [fakeWord1, fakeWord2, fakeWord3, fakeWord4, fakeWord5]; //Array aller vorhandenen Falschen Worte
let globalActiveFakeWords: FakeWord[] = []; //Array der im aktiven atz genutzen FalschenWorte (Wird jeden Satz zufällig neu geüllt und danach wieder geleert)

/*Häufig genutzte HTML-Elemente */
let globalGeneralGameArea: HTMLElement = document.querySelector(".generalGameArea"); //gesamter Spielbereich
let globalLowerGameArea: HTMLElement = document.querySelector(".lowerGameArea"); //Bereich der Fremdworte
let globalCentralGameArea: HTMLElement = document.querySelector(".centralGameArea"); //Bereich der korrekt geählten Worte
let globalUpperGameArea: HTMLElement = document.querySelector(".upperGameArea"); //Bereich der Deutschen Worte
...
function setDifficulty()
{
    ...
    setTimeout(function() //Entferne das Menu-Div nach 1s aus dem DOM-Tree (Wird entfernt nachdem es unsichtbar geworden ist, fade out effect)
    {
...
function createGermanSentence(currentSentence)
{
    ...
    for(let i = 0; i < currentSentence.length; i++)
    {
        ...
        setTimeout(function() //setTimeout notwendig da sonst die transition Eigenschaft im CSS nicht "greift"
        {
...
function createFakeWordsPool(poolSize: number)
{
    ...
    {
        let currentFakeWord: FakeWord = globalAllFakeWords[getRandomNumberBetween(0, (globalAllFakeWords.length - 1))]; //Wähle zufälligs Falsches Wort aus Array aller falscher Wörter aus
...
function checkWord()
{
    ...
    { //Wenn ja: 
        ...
        setTimeout(function() //warte 1,5s damit der "Wort korrekt"-Sound abgespielt wurde
        {
...
function moveCorrectWord(activeWord)
{
    ...
    setTimeout(function() //timeout damit der rest der Funktion erst durchgeführt wird wenn der fadout abgeschlossen ist
    {
        ...
        setTimeout(function() //otwednig damit die transition eigenschaft von CSS "greift"
        { //Mach das aktive Wort im neuen Bereich wieder sichtbar und setze Hintergrund und Schriftfarbe auf normal zurück
...
function checkForEndOfSentence()
{
    ...
    { //Wenn satz nnicht zu Ende springe aus der Funktion
...
{ 
    ...
    { 
        ...
        setTimeout(function() //Warte bis das Rolldown Menu erzeugt wurde
        {
